...
# ══════════════════════════════════════════════════════════════════
# LIVE DIGITAL TWIN ROUTES
# ══════════════════════════════════════════════════════════════════
# Flow: describe the person → follow-up questions → build their twin →
# live video/voice conversation → LLM judges the user's real performance
# using live facial / body / eyebrow / voice signals captured during the call.

def _live_mgr():
    from agent.live.live_session_manager import get_live_session_manager
    return get_live_session_manager()


@app.route("/use-cases", methods=["GET"])
def use_cases_list():
    """GET /use-cases → the 10 practice scenarios shown right after login."""
    from agent.live.use_cases import list_use_cases
    return jsonify({"use_cases": list_use_cases()})


@app.route("/streak/<use_case_id>", methods=["GET"])
@require_auth
def streak_status(use_case_id):
    """GET /streak/<use_case_id> → this user's Vyakti Streak status for one use case."""
    from agent.live.streak_manager import get_streak_manager
    return jsonify(get_streak_manager().get_status(request.user_id, use_case_id))


@app.route("/streak", methods=["GET"])
@require_auth
def streak_status_all():
    """GET /streak → this user's Vyakti Streak status across all use cases they've practiced."""
    from agent.live.streak_manager import get_streak_manager
    return jsonify({"streaks": get_streak_manager().get_all_status(request.user_id)})


@app.route("/live/questions", methods=["POST"])
@require_auth
def live_questions():
    """POST { description, use_case_id? } → { session_id, questions, use_case_id, level }"""
    data = request.get_json(force=True)
    description = (data.get("description") or "").strip()
    if not description:
        return jsonify({"error": "description is required"}), 400
    result = _live_mgr().start_description(
        description, user_id=request.user_id, use_case_id=data.get("use_case_id")
    )
    return jsonify(result)


@app.route("/live/build", methods=["POST"])
@require_auth
def live_build():
    """POST { session_id, answers: {q_id: answer} } → { persona, opening_line }"""
    data = request.get_json(force=True)
    session_id = data.get("session_id")
    if not session_id:
        return jsonify({"error": "session_id is required"}), 400
    try:
        result = _live_mgr().build_twin(session_id, data.get("answers", {}))
    except KeyError:
        return jsonify({"error": "Session not found"}), 404
    return jsonify(result)


@app.route("/live/message", methods=["POST"])
@require_auth
def live_message():
    ...
    try:
        result = _live_mgr().send_message(session_id, message)
    except KeyError:
        return jsonify({"error": "Session not found"}), 404
    return jsonify(result)
...
@require_auth
def live_frame():
    """POST { session_id, image } (base64 data URL) → { ok }. Fire-and-forget from the browser."""
    data = request.get_json(force=True)
    session_id = data.get("session_id")
    ...
    try:
        ok = _live_mgr().ingest_frame(session_id, image)
    except KeyError:
        return jsonify({"error": "Session not found"}), 404
...
@require_auth
def live_audio():
    """POST { session_id, audio, mimetype? } (base64) → { ok }. Fire-and-forget from the browser."""
    data = request.get_json(force=True)
    session_id = data.get("session_id")
    ...
    suffix = ".wav" if "wav" in mimetype else ".webm" if "webm" in mimetype else ".ogg" if "ogg" in mimetype else ".webm"
    try:
        result = _live_mgr().ingest_audio(session_id, audio, suffix=suffix)
    except KeyError:
        return jsonify({"error": "Session not found"}), 404
...
@require_auth
def live_end():
    """POST { session_id } → full performance judgment."""
    data = request.get_json(force=True)
    session_id = data.get("session_id")
    if not session_id:
        return jsonify({"error": "session_id is required"}), 400
    try:
        judgment = _live_mgr().end_session(session_id)
    except KeyError:
        return jsonify({"error": "Session not found"}), 404
    return jsonify(judgment)


@app.route("/live/session/<session_id>", methods=["GET"])
@require_auth
def live_session_status(session_id):
    """GET /live/session/<id> → current phase, persona, conversation, judgment (if any)."""
    session = _live_mgr().get_session(session_id)
    if not session:
        return jsonify({"error": "Session not found"}), 404
