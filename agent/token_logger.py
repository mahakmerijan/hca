...
def log_call(
    ...
    # Fall back to the active run_context's run_id so log_run_summary(sim_id=...)
    # aggregation works for every caller without each one having to pass sim_id.
    if sim_id is None:
        sim_id = context.get("run_id", "")
